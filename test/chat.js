...
wait_cluster_up(up_timeout, function(status, nodes, elapsed) {
    if (status) {
        console.log('Cluster is up after ' + elapsed + 'ms');
        //show_nodes(nodes);

        console.log('Sending chat message via last page/server');
        pages[pages.length-1].evaluate(function() {
            $('#talk').val("test line");
            //sendLine();
            $('#send').click();
        });
        wait_cluster_predicate(function () {
        ...
        }, pred_timeout, function(status, results, elapsed) {
            if (status) {

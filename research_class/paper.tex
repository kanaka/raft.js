...
\author{Joel D. Martin and David Levine
\thanks{University of Texas at Arglington}
}
...
implemenation (e.g. assigning client IDs and sequence IDs and tracking
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.8\textwidth]{raft_rtc_architecture}}
...
\begin{figure*}[h]
\centerline{\includegraphics[width=0.8\textwidth]{raft_rtc_sequence}}
...
\item Dynamic Raft.js over WebRTC: enhance the signaling server with support for multiple sessions/channels to enable separate Raft clusters with the same signalling server. Start each cluster with a single node and automatically add and remove Raft nodes as they come and go.
...
\subsubsection{Client Request Implementation}

TODO: client request functionality
...
\emph{Send} button (or pressed enter), the text line is added to
...
TODO: atomic seqAppend operation

TODO: longer delay for propagation in chat application because each
addition to chat history take several messages to fully propagate.
...
\begin{center}
    \includegraphics[width=0.4\textwidth]{browser_1.png}
    \captionof{figure}{Raft over WebRTC Browser with One Node}
    \label{fig:browser_1}
\end{center}
...
\begin{center}
    \includegraphics[width=0.4\textwidth]{browser_2.png}
    \captionof{figure}{Raft over WebRTC Browser with Five Nodes}
    \label{fig:browser_2}
\end{center}

In addition, the full chat application was also tested manually. TODO:
more text.

TODO: image of chat app with one node

TODO: image of chat app five nodes
...
server from the cluster) when the current leader fails.
...
propagate, commit and apply a single state machine command to the state
